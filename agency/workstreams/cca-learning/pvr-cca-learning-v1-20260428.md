...
- **FR3.8** **`[CHANGE]`** **Coordinator-subagent generation pipeline** — original spec promised a 3-subagent decomposition for question generation (D1.2 dogfooding) but the implementation collapsed to single-generator + reviewer. This PVR commits to refactoring the generator into the coordinator-subagent pattern: three specialized subagents (a) scenario-context writer, (b) rubric drafter, (c) model-answer drafter, plus a coordinator that synthesizes their outputs into the final question candidate. Reviewer pass (FR3.5) runs on the synthesized output. Cost: ~2.5× more Claude calls per question (3 subagents + coordinator + reviewer ≈ 5 calls) and proportional latency. Payoff: real D1.2 dogfooding rather than checkbox dogfooding. **Not yet implemented** — see Open Question 9.b.
...
- **9.C.2 — PVR evolution model.** This PVR is a **fixed v1 snapshot**. It is amended only to mark Group A items resolved or Group B items verified — never to add new requirements. New requirements live in v2's PVR. `/design` and `/plan` work against this stable v1 PVR; the framework expects this discipline.
