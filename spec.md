...
- **AT20 — Cache Hit-Rate Panel** (E1): With ≥ 10 logged calls per cache-enabled role, `/spend` renders a per-role hit rate computed as `cache_read / (cache_read + cache_creation + uncached_input)`. Roles declared `cacheSystem: true` (tutor, generator, explainer, card-writer, rubric-drafter) show a hit rate; roles declared `cacheSystem: false` are labelled "no-cache" and omitted from the warning logic. A role with `cacheSystem: true` but hit rate < 50% over ≥ 10 calls renders an amber pill so the operator can investigate.
- **AT21 — MCQ Re-Test Queue** (E2): Answering 5 distinct MCQs incorrectly creates 5 `mcq_review_state` rows with staggered `due_at` timestamps; the dashboard surfaces "5 MCQs due"; opening `/drill?scope=due-mcq` returns those exact items in due-soonest order. Subsequent correct answers advance the SM-2 interval; subsequent failures reset to interval=0.
- **AT22 — Bullet-Level Coverage** (E3): `/admin/coverage` renders a "Bullet blind spots" section listing every `(taskStatementId, bulletKind, bulletIdx)` tuple with zero active questions referencing it. The generator pipeline rejects any candidate whose declared `knowledge_bullet_idxs` or `skills_bullet_idxs` are out of range for the target task statement.
- **AT23 — Elo Calibration** (E4): A question answered correctly by ≥ 80% of users with sufficient attempts (`attempts_count ≥ 5`) settles to a rating ≥ 200 Elo below the user's cell rating; running a drill with `targetSuccessRate=0.7` no longer surfaces it. Conversely, a question consistently missed converges to a rating ≥ 200 Elo above the user's cell rating.
- **AT24 — MCP Curriculum Server** (E5): With the MCP server registered, a Claude Code session can call `read_curriculum({ taskStatementId: "1.2" })` and receive verbatim Knowledge/Skills bullets, plus a `read_progress({ taskStatementId: "1.2" })` call returns the same Bloom-level snapshot rows the app reads. Both tools are read-only; mutations are absent from the tool surface.

## v1.1 Enhancements

These are post-Phase-13 enhancements added once the v1 spec was demonstrably satisfied. Each one extends an existing FR/NFR rather than adding net-new requirements; each maps to an acceptance test (AT20–AT24) and a phase in `tasks/todo.md` (Phases 14–18). They are tracked here so spec.md remains the source of truth for the project's behavioral surface.

- **E1 — Prompt-cache hit-rate observability** (extends NFR4.3, FR5.4). The call log already captures `cache_creation_input_tokens` and `cache_read_input_tokens`; surface them on `/spend` as a per-role hit rate so we can verify NFR4.3 ("prompt caching is used where supported") is actually paying off in production. Roles whose role definition declares `cacheSystem: true` (tutor, generator, explainer, card-writer, rubric-drafter) get a hit rate and an amber-pill warning when the rate is < 50% over a meaningful sample. Verified by **AT20**, implemented in **Phase 14**.
- **E2 — SRS re-test for missed MCQs** (extends FR2.3, FR4). Treat every missed MCQ as a retrieval-practice opportunity by feeding it through the same SM-2 scheduler that drives flashcards (`lib/progress/sm2.ts`). Adds `mcq_review_state` table; on every `mcq_answer` event, upsert the row with quality=4 on success / quality=1 on failure. Adds drill scope `{ type: "due-mcq" }` and a dashboard "MCQs due" tile. Verified by **AT21**, implemented in **Phase 15**. Decision deferred to phase: paraphrase-on-review (cheap-model) vs. verbatim repeat — start verbatim, measure transfer, revisit.
- **E3 — Bullet-level coverage tracking** (extends FR3.7). Today's coverage matrix ((TS, Bloom)) hides bullet-level blind spots — one Knowledge bullet may have 5 questions while a sibling has 0. Adds `questions.knowledge_bullet_idxs` and `questions.skills_bullet_idxs` JSON arrays; generator must declare which bullets each question tests; reviewer rejects out-of-range indices; coverage report grows a "bullet blind spots" view. Verified by **AT22**, implemented in **Phase 16**.
- **E4 — Elo question/user calibration** (extends FR3.4, FR4). The generator-self-reported `difficulty` field is a stale prior; replace with a Glicko-1 posterior maintained on every MCQ event. Adds Elo + volatility columns on `questions`, a per-cell `user_skill` table, and an optional `targetSuccessRate` (default 0.7) on the drill allocator so questions are picked from the desirable-difficulty zone. Mock allocator stays exam-realistic by preferring items within ±200 Elo of the user's domain-cell mean. Verified by **AT23**, implemented in **Phase 17**.
- **E5 — MCP server over the curriculum DB** (activates D2.4, deferred in v1 — see "Dev-Time Claude Code Setup"). Stands up `.mcp.json` with read-only `read_curriculum` and `read_progress` tools (and `cca://task-statement/{id}` resources) so Claude Code sessions can query the same SQLite the app does. Read-only is non-negotiable to prevent stray sessions from mutating user progress. Verified by **AT24**, implemented in **Phase 18**.
